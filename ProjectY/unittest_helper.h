/*
    modify the rnnoise_demo2.c

    Contain these processing:
    1. [-] wavfile read/write
    2. [-] time consume calculate
    3. profile
*/
...
// open wav file
drwav* open_wavfile(const char* filename, uint16_t* channelsOut, uint16_t* sampleRateOut, uint64_t* totalPCMFrameCount){
    if (channelsOut) *channelsOut = 0;
    if (sampleRateOut) *sampleRateOut = 0;
    if (totalPCMFrameCount) *totalPCMFrameCount = 0; // totalSampleCountOut = totalPCMFrameCount * channelsOut
    ...
    *channelsOut = pWav->channels;
    *sampleRateOut = pWav->sampleRate;
    *totalPCMFrameCount = pWav->totalPCMFrameCount;
    return pWav;
}
